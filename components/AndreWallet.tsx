...
const AndreWallet: React.FC<Props> = ({ 
  coupleInfo, 
  expenses, 
  monthKey, 
  onAddExpense,
  ...
  const andreLeftToInvest = coupleInfo.salary1 - summary.person1Responsibility - summary.person1PersonalTotal;

  const [targetYear, targetMonth] = monthKey.split('-').map(Number);
  const personalExpenses = expenses.filter(exp => {
    if (exp.type !== ExpenseType.PERSONAL_P1) return false;
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right duration-500 pb-24">
      ...
      <div className="bg-white p-6 md:p-8 rounded-[2rem] border-2 border-dashed border-blue-200 shadow-sm space-y-8 relative overflow-hidden group">
        <div className="flex items-center justify-between z-10 relative">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-black text-gray-900 text-3xl tracking-tighter">Carteira do {coupleInfo.person1Name}</h2>
              <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Resumo Financeiro Pessoal</p>
            </div>
          </div>
          <button 
            onClick={() => setShowAdd(true)}
            className="bg-blue-600 text-white p-4 rounded-2xl shadow-xl active:scale-95 transition"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
            ...
            <p className="text-2xl font-black text-gray-800">{formatCurrency(coupleInfo.salary1)}</p>
          </div>

          <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Cota Compartilhada + Extras</p>
            <p className="text-2xl font-black text-red-500">-{formatCurrency(summary.person1Responsibility + summary.person1PersonalTotal)}</p>
          </div>

          <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100">
            <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Poder de Aporte (Livre)</p>
            <p className="text-3xl font-black text-blue-600 tracking-tighter">{formatCurrency(andreLeftToInvest)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b bg-gray-50/50 flex justify-between items-center">
          ...
          <span className="font-bold text-gray-900">{formatCurrency(summary.person1PersonalTotal)}</span>
        </div>
              ...
              <div key={exp.id} className="p-5 flex justify-between items-center group hover:bg-gray-50 transition">
                <div>
                   <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-lg font-black uppercase tracking-widest">{exp.category}</span>
