...
const BalanceCard: React.FC<BalanceCardProps> = ({ name, personal, left, color }) => {
    ...
    const accentIconBg = color === 'blue' ? 'bg-blue-600' : 'bg-pink-500';

    return (
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm group hover:shadow-xl transition-all duration-500 relative overflow-hidden">
            {/* Background Decor */}
            ...
            <div className="flex items-center justify-between mb-8 relative">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-black text-slate-900 tracking-tight">{name.split(' ')[0]}</h3>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Resumo de Carteira</p>
                    </div>
                </div>
            </div>

            <div className="grid grid-cols-2 gap-4 relative">
                <div className="p-6 bg-slate-50/50 rounded-3xl border border-slate-100/50 group-hover:bg-white transition-colors">
                ...
                <div className={`p-6 ${accentBg} rounded-3xl border border-transparent transition-all group-hover:bg-white group-hover:border-slate-100`}>
