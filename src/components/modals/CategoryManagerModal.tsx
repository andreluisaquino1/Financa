...
const CategoryManagerModal: React.FC<Props> = ({ coupleInfo, onSave, onClose }) => {
    const [categories, setCategories] = useState<Category[]>(() => {
        const initialCats = (coupleInfo.categories || []).map(c => typeof c === 'string' ? { name: c } : c);
        return initialCats.length > 0 ? initialCats : DEFAULT_CATEGORIES;
    });
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in duration-300 border border-white/10">
                <div className="px-8 py-6 border-b dark:border-white/5 flex justify-between items-center bg-white dark:bg-slate-900 z-10 shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-brand/10 flex items-center justify-center text-xl shadow-inner border border-brand/5">🏷️</div>
                        <div>
                            <h3 className="font-extrabold text-slate-900 dark:text-slate-100 tracking-tight text-lg">Categorias de Gasto</h3>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">Organização Financeira</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
                    <div className="bg-slate-50 dark:bg-slate-950/40 p-6 rounded-3xl border border-slate-200 dark:border-white/5 space-y-4">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Nova Categoria</h4>
                        <div className="flex gap-2">
                            ...
                            <input
                                ...
                                placeholder="Ex: Farmácia, Cinema..."
                            />
                            <button
                                onClick={handleAddCategory}
                                disabled={!newCategory.trim()}
                                className="bg-brand text-white w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg shadow-brand/20 active:scale-95 transition-all text-2xl font-bold disabled:opacity-50"
                            >
                                +
                            </button>
                        </div>
